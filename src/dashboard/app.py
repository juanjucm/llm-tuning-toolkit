...
def run(from_results_dir, datasource, port):
    ...
    * Error rate: The percentage of requests that ended up in error, as the system could not process them in time or failed to process them. 
    ...
    def summary_table() -> pd.DataFrame:
        data = (
            df_bench.groupby(["model", "run_id", "rate"])
            .agg(
                {
    ...
    def select_region(selection: gr.SelectData, model):
        ...
        data = df_bench[
            (df_bench["model"] == model) & (df_bench["rate"] >= min_w) & (df_bench["rate"] <= max_w)
        ]
        res = []
        for plot in line_plots_bench:
    ...
    # Define metrics
    metrics = {
        "inter_token_latency_ms": PlotConfig(
            title="Inter Token Latency (lower is better)",
            x_title="QPS",
            y_title="Time (ms)",
            percentiles=[0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.99],
        ),
        "time_to_first_token_ms": PlotConfig(
            title="TTFT (lower is better)",
            x_title="QPS",
            y_title="Time (ms)",
            percentiles=[0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.99],
        ),
        "e2e_latency_ms": PlotConfig(
            title="End to End Latency (lower is better)",
            x_title="QPS",
            y_title="Time (ms)",
            percentiles=[0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.99],
        ),
        "token_throughput_secs": PlotConfig(
            title="Request Output Throughput (higher is better)", x_title="QPS", y_title="Tokens/s"
        ),
        "successful_requests": PlotConfig(
            title="Successful requests (higher is better)", x_title="QPS", y_title="Count"
        ),
    ...
    with gr.Blocks(css=css, title="Inference Benchmarker") as demo:
        ...
        for component in [model, percentiles_bench]:
            component.change(
                update_bench,
                [model, percentiles_bench],
                [item["component"] for item in line_plots_bench] + [table],
            )
        gr.on(
            [plot["component"].select for plot in line_plots_bench],
            select_region,
            [model],
            outputs=[item["component"] for item in line_plots_bench],
        )
        gr.on(
            [plot["component"].double_click for plot in line_plots_bench],
            reset_region,
            None,
            outputs=[item["component"] for item in line_plots_bench],
        )
        demo.load(
            load_demo, [model, percentiles_bench], [item["component"] for item in line_plots_bench] + [table]
        )
